...
export async function GET(request) {
    try {
        const response = await fetch(`${BACKEND_API_URL}/api/promotions/current`, {
            headers: {
        ...
        console.error("Current promotions proxy error:", error);

        return NextResponse.json(
            { success: false, message: "Unable to reach the promotions service." },
            { status: 502 }
        );
    }
}
